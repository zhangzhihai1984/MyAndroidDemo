...
//                }
        ...
        recyclerview.adapter = RowAdapter(listOf(arrayListOf(
    ...
    private class RowViewHolder(view: View) : BaseViewHolder(view) {
        companion object {
        ...
        init {
            view.findViewById<RecyclerView>(R.id.seat_recyclerview).run {
                layoutManager = GridLayoutManager(context, SEAT_COUNT_PER_ROW, RecyclerView.VERTICAL, false)
                adapter = seatAdapter
                globalLayouts()
                        .take(1)
                        .`as`(RxUtil.autoDispose(context as LifecycleOwner))
                        .subscribe { updateLayoutParams { height = getWidth() / SEAT_COUNT_PER_ROW - context.resources.getDimensionPixelSize(R.dimen.selection_item_margin) * 2 } }
            }
    ...
    private class RowAdapter(data: List<List<Status>>) : RxBaseQuickAdapter<List<Status>, RowViewHolder>(R.layout.item_seat_selection_row, data) {
