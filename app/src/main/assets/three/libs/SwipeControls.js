...
THREE.SwipeControls = function( camera, domElement ) {
	...
	this.update = function() {
		...
		if ( mouseDown ) {
			...
			camera.position.x += deltaMouseX * this.speed;
			camera.position.y -= deltaMouseY * this.speed;
	...
	function ensureLimits() {
		...
		} else if ( camera.position.x < leftLimit ) {
			camera.position.x = leftLimit;
		}

		if ( camera.position.y > topLimit ) {
			camera.position.y = topLimit;
		} else if ( camera.position.y < bottomLimit ) {
